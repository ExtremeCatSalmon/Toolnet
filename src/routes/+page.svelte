<script lang="ts">
    ...
    const emptyNodeIO = { inputs: {}, outputs: {}, props: {} };
            ...
            if (msg.type === "ready") {
                ...
                for key,value in pairs(package.preload) do
                    if string.find(key,"^nodes.") then
                        names[key] = {}
                        names[key]['inputs'] = value().inputs or {}
                        names[key]['outputs'] = value().outputs or {}
                        names[key]['props'] = value().props or {}
                    end
                end
                return names
                `,
    ...
    function makeTreeFrom(nodeId: number, output: string = ""): NodeTreeNode {
        let treeRoot: NodeTreeNode = {
            ...
            output,
            props: $nodes.find((n) => n.id === nodeId)?.props ?? {},
    ...
    function luaStringify(data: any): any {
        if (typeof data === "object") {
            let pairs: string[] = [];
            for (let [k,v] of Object.entries(data)) {
                if (!isNaN(k)) {
                    pairs.push(`${luaStringify(v)}`);
                } else {
                    pairs.push(`${k}=${luaStringify(v)}`);
                }
            }
            return "{"+pairs.join(',')+"}";
    ...
    async function runTree(tree: NodeTreeNode): Promise<string> {
        ...
        for (const [input, childTree] of Object.entries(tree.connectedNodesByInputPort)) {
            let data = await runTree(childTree);
            args.push(`${input}=${luaStringify(data[0][childTree.output])}`);
        }

        let prop_code = '';
        for (const [k,v] of Object.entries(tree.props)) {
            const value = luaStringify(v);
            prop_code+=`mod['${k}']=${value}\n`;
        }
        const code = `
        local mod = require "${tree.module_name}"
        ${prop_code}
        return mod({${args}})
        `;
        console.log(code);
    ...
    >
        {#each $nodes as node (node.id)}
            <Node
                ...
                onpropsupdate={(props: Record<string, string>) => {
                    $nodes = $nodes.map((n) =>
                        n.id === node.id ? { ...n, props } : n,
                    );
                }}
